...
// --- turn OFF default Cesium popup + selection indicator ---
...
viewer.camera.moveEnd.addEventListener(() => {
    clearTimeout(timer);
    timer = setTimeout(loadBooksInView, 300);
});
...
async function loadBooksInView() {
    ...
    // --- FIX: padding for stable results ---
    const pad = Cesium.Math.toRadians(2); // 2-degree padding
...
function renderMarkers(books) {
    ...
    books.forEach((book) => {
        (book.locations || []).forEach((loc) => {
            ...
            // Remember which book belongs to this entity
...
function showPopup(book, position) {
    ...
    if (windowPos) {
        popupEl.style.left = windowPos.x - 130 + "px"; // center horizontally
...
// Close popup when clicking on empty map
...
function renderBooks(books) {
    ...
    countEl.innerText = `${books.length} books`;
    list.innerHTML = "";
        ...
        div.innerHTML = `
            <div class="book-card-inner">
                ...
                <div class="book-meta">
                    ...
                    <div class="book-author">${book.author || ""}${
                        book.year ? " • " + book.year : ""
                    }</div>
...
addBookModal.querySelectorAll(".modal-close").forEach((btn) =>
    btn.addEventListener("click", closeModal)
);
addBookModal.querySelector(".modal-backdrop").addEventListener("click", closeModal);
